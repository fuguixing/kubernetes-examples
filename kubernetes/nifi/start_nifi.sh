...
do_authentication_configure() {
  ...
  sed -i "s/nifi\.security\.needClientAuth=.*/nifi.security.needClientAuth=true/g" ${NIFI_HOME}/conf/nifi.properties
...
do_cluster_node_configure() {
  ...
  sed -i "s/nifi\.web\.https\.host=.*/nifi.web.https.host=${HOSTNAME}/g" ${NIFI_HOME}/conf/nifi.properties
  sed -i "s/nifi\.web\.http\.host=.*/nifi.web.http.host=/g" ${NIFI_HOME}/conf/nifi.properties
  ...
  sed -i "s/nifi\.cluster\.is\.node=false/nifi.cluster.is.node=false/g" ${NIFI_HOME}/conf/nifi.properties
